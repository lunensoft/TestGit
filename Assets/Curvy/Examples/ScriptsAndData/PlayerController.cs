...
public class PlayerController : MonoBehaviour {
    ...
    void Update()
    {
        ...
        if (moveaxis != 0) {
            ...
            Vector3 newPos = Spline.MoveBy(ref TF, ref newdir, Mathf.Abs(moveaxis) * Speed * Time.smoothDeltaTime, CurvyClamping.PingPong);
            // We need to handle spline ends. If direction changed, we reached an end, so switch the rail spline
            if (newdir != dir) {
                // Get the next spline to move on
                Spline = Spline.GetComponent<NextRailSpline>().NextSpline;
                // Get the tf we need to move on the new spline
                TF = Mathf.Abs(1 - TF);
                // Move that delta on the new spline
                newdir = dir;
                newPos = Spline.MoveBy(ref TF, ref newdir, Mathf.Abs(moveaxis) * Speed * Time.smoothDeltaTime, CurvyClamping.PingPong);
            }
